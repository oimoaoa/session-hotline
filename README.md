> ## ⚠️ 이 저장소는 보관(archive) 상태예요 — 더 이상 유지보수하지 않습니다
>
> **2026-08-18부터 읽기 전용**이에요. 마지막 코드 업데이트는 **2026-07-13**이고, 그 뒤로는 관리하지 못했어요.
>
> - **새로 설치하는 건 권하지 않아요**. 완전히 죽지는 않았지만 **그대로 쓰기 어렵고 손으로 우회해야 하는 부분이 많아요**. 이 스킬은 Claude Code·Codex 데스크탑 앱의 내부 파일 구조에 기대는 구조라, 앱이 업데이트되면 어긋날 수 있다는 게 원래 알려진 제약이었어요([이슈 #3](https://github.com/oimoaoa/session-hotline/issues/3)).
> - **이슈·PR은 받지 않아요** — 보관 상태라 잠깁니다. 답을 못 드릴 창구를 열어두는 게 더 실례라고 생각했어요.
> - **가져다 쓰는 건 언제든 환영이에요**. MIT 라이선스 그대로라 fork 떠서 자유롭게 고쳐 쓰셔도 됩니다. 아이디어나 코드 조각이 도움이 된다면 그걸로 충분해요.
>
> 아래 문서는 **2026-07-13 시점 기준 기록**으로 남겨둬요. 지금 동작과 다를 수 있어요.
...
**한눈에 —**
- 🗣️ 지금 대화방에서 한 문장이면, **다른 AI 대화방에 대신 물어보고 답을 정리**해와요
- 🔀 **Claude Code ↔ Codex** 넘나들며 질문·답 — 한 번 설치로 양쪽 공용
- 🔒 안전장치 내장 — 보내기 전 되물어 확인 · 읽기 전용 · 시크릿 스캔
...
```
https://github.com/oimoaoa/session-hotline 이 주소의 스킬이 뭔지 파악해서 나한테 쉬운 말로 설명해주고, 내 환경에 맞게 설치해줘. 설치는 저장소의 install.sh를 써줘 — 복사가 아니라 심볼릭 링크(symlink)로 깔려서, 나중에 git pull로 업데이트할 수 있어.
```

AI가 내용을 읽고, 뭐가 설치되는지 설명해 주고, 설치까지 도와줘요. 중간에 **macOS 권한 승인 창이 뜰 수 있는데 정상**이에요. 혹시 실패하면 그 **에러 메시지를 그대로 AI에게** 보여주면 돼요.

> 💡 설치가 **심볼릭 링크**로 되니까, 나중에 최신 버전을 받는 건 내려받은 폴더에서 `git pull` 한 번이면 돼요(아래 **🔄 업데이트** 참고).
...
> 💡 등록은 **복사가 아니라 심볼릭 링크(symlink)** 예요 — 그래서 나중에 이 폴더에서 `git pull` 한 번이면 재설치 없이 최신이 반영돼요(아래 **🔄 업데이트** 참고). 대신 **내려받은 폴더(`~/session-hotline`)가 실행 원본**이니 지우거나 옮기지 마세요.
...
> ⚠️ **복사(copy)가 아니라 심볼릭 링크(`ln -s`)로 연결하세요.** 복사하면 나중에 `git pull` 해도 사본은 옛날 그대로라 업데이트가 안 돼요.

1. 이 저장소를 **계속 둘 폴더**에 clone(내려받기) — 예: `~/session-hotline` (이 폴더가 실행 원본이 되니 지우지 마세요)
2. `hotline.sh`·`resolve_claude.py`·`resolve_codex.py` **세 파일을 `~/.session-hotline/`에 링크**(같은 폴더에 있어야 대화방을 제목으로 찾아요) + `hotline.sh`에 실행 권한:
   ```bash
   cd ~/session-hotline
   mkdir -p ~/.session-hotline
   for f in hotline.sh resolve_claude.py resolve_codex.py; do ln -sfn "$PWD/$f" ~/.session-hotline/"$f"; done
   chmod +x hotline.sh
   ```
3. `skills/SKILL.md`를 링크 — Claude Code용:
   ```bash
   mkdir -p ~/.claude/skills/session-hotline && ln -sfn "$PWD/skills/SKILL.md" ~/.claude/skills/session-hotline/SKILL.md
   ```
4. 같은 파일을 Codex용으로도 링크(코덱스를 쓴다면):
   ```bash
   mkdir -p ~/.codex/skills/session-hotline && ln -sfn "$PWD/skills/SKILL.md" ~/.codex/skills/session-hotline/SKILL.md
   ```

</details>

> **고급 사용자용** — 설치는 **복사가 아니라 심볼릭 링크(symlink)** 라, clone한 폴더가 곧 실행 원본이고 업데이트는 그 폴더에서 `git pull` 한 번이면 끝이에요(아래 **🔄 업데이트** 참고). 코드를 직접 고쳐 쓰고 싶으면 fork해서 **본인 저장소**를 clone하세요(그러면 `git pull`이 본인 fork 기준이라 충돌이 안 나요). 예전에 **복사**로 설치했던 경우엔 `install.sh`를 한 번 다시 돌리면 링크로 바뀌고(기존 파일이 다르면 자동 백업), 그다음부터 `git pull`로 업데이트돼요.

## 🔄 업데이트

이 도구는 **심볼릭 링크(symlink)로 설치**되므로, 최신 버전을 받는 건 clone한 폴더에서 `git pull` 한 번이면 돼요:

```bash
cd ~/session-hotline && git pull
```

*(다른 위치에 clone했다면 `~/session-hotline` 대신 그 경로로.)*

- **재설치가 필요 없어요.** 링크는 "clone 폴더 안 원본 파일을 가리키는 바로가기"라, `git pull`로 그 파일이 갱신되면 앱이 다음 실행부터 자동으로 최신 코드를 써요.
- **자동으로 오지는 않아요.** GitHub은 새 버전이 나왔다고 알려주지 않으니, 업데이트를 받으려면 위 `git pull`을 **직접** 실행해야 해요(가끔 확인해 주세요). 알림은 따로 없어요.
- **fork(포크)는 필요 없어요.** 그냥 `git clone`만 했으면 이 저장소가 `origin`으로 잡혀 있어 `git pull`로 바로 최신을 받아요. fork는 직접 코드를 고쳐 올리거나 수정을 제안(PR)할 때만 필요해요.

**🙋 개발 몰라도 돼요** — 아래 한 줄을 클로드나 코덱스 채팅창에 그대로 붙여넣으면 대신 업데이트해줘요:

```
session-hotline을 최신으로 업데이트해줘. 내가 clone한 폴더(기본 ~/session-hotline)에서 git pull을 실행하고, 최신을 잘 받았는지 확인해줘. 폴더 위치를 모르면 나한테 먼저 물어봐줘.
```

> ⚠️ **clone한 폴더(예: `~/session-hotline`)를 지우거나 옮기면 링크가 끊겨요** — 그 폴더가 실행 원본이라서예요. 옮겼다면 그 위치에서 `install.sh`를 한 번 다시 돌리면 링크가 다시 걸려요. 파일 이름·구조가 바뀌는 큰 업데이트가 아니면 링크는 그대로 유지돼요.
...
설치되는 곳은 세 군데예요. 아래 한 줄로 흔적 없이 제거돼요:

```bash
rm -rf ~/.session-hotline ~/.claude/skills/session-hotline ~/.codex/skills/session-hotline
```

> 이건 **링크(바로가기)만** 지워요 — 원본 파일은 clone한 폴더(예: `~/session-hotline`)에 그대로 남아요. 완전히 없애려면 그 폴더도 지우세요(`rm -rf ~/session-hotline`).
